...
object Interaction extends InteractionInterface {
  ...
  : Image = {
    val locations = for {
      imageTile <- tile
    } yield tileLocation(imageTile)
    Visualization.visualizeLocTemps(locations, temperatures, colors, 256, 256, 127)
  }
  ...
  def tileLocation(tile: Tile): Location = {
    val lat = math.toDegrees(atan(sinh(Pi * (1.0 - 2.0 * tile.y.toDouble / tile.N))))
    val lon = (tile.x.toDouble / tile.N) * 360 - 180
    Location(lat, lon)
  }
